...
/**
 *  \brief SPI Command Definition Byte Assignments.
 */ 
...
/**
 *  \brief Register addresses for ADS1299.
 */ 
...
#define ADSspi_CSlow()   PORTD.OUTCLR = PIN4_bm           //define for Chip Select low
#define ADSspi_CShigh()  PORTD.OUTSET = PIN4_bm           //define for Chip Select high
...
void adsRREG(byte _address, byte _numRegisters, byte* data); //to read multiple consecutive registers 
...
bool ADSnewData; // set this True within ISR before calling adsGetData()
long singleSample[9]; // a single sample of all 8 channels and status byte
